...
`main` 브랜치에 푸시하면 GitHub Pages로 배포됩니다. 정적 파일 캐시를 쓰기 때문에 JS, CSS, 서비스워커를 바꿀 때는 변경된 파일의 버전 query와 `sw.js`의 `CACHE_NAME`을 같이 갱신합니다.

파일별 버전 query는 유지합니다. 전체를 하나의 버전으로 묶지 않고, 바뀐 파일만 갱신해 캐시 무효화 범위를 작게 가져갑니다.
