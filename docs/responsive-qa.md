...
- 성능: 홈 미리보기 영역 분석 캐시는 작품 버전을 키에 포함하고, 저장/조회 시 defensive copy와 크기 제한을 사용하도록 회귀 테스트로 확인
- 성능: 홈 완성 썸네일 자동 채색 캐시는 작품 버전과 이미지 src를 키에 포함해 도안 교체 시 이전 캐시를 재사용하지 않고, 저장/조회 시 defensive copy와 크기 제한을 사용하도록 회귀 테스트로 확인
